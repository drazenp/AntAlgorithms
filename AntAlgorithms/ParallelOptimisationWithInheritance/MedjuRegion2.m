...
%---------======I=======----------------------------
%%%%%%%%%%%%% Kljucni parametrri%%%%%%%%%
% MaxBrIteracija=10; % broj iteracija
% BrRegiona = 3; % broj mrava = broj regiona
alfa=1; % znacaj debljine feromonskog traga
beta=5; % znacaj povezanosti cvorova
ro=0.7; % faktor isparavanja feromona
% tol = 0.1; % tolerancija
% BrMedjuPodela=5; % broj podela u jednoj iteraciji
%%%%%%%%%%%%% Kljucni parametri %%%%%%%%%

dobrota=ASPG(TC,TG,MaxBrIteracija,BrRegiona,alfa,beta,ro,tol,BrMedjuPodela);

function kvalitet=ASPG(TC,TG,MaxBrIteracija,BrRegiona,alfa,beta,ro,tol,BrMedjuPodela)
...
tic
	...
	while 1
		...
		if ASPGOpcije.GNVelicina>-Inf
			SistemMrava.Putovanja(:,:,BrMedjuPodela)=ASPGOpcije.GNPutanja;
			ASPGOpcije.MedjuPodela=BrMedjuPodela-1;
		end
		...
		SumaTezinaTacaka(TC,BrMedjuPodela);
		...
		IVreme=IVreme+1;
		TezinaGranaPovezanosti(BrMedjuPodela);
		SumaKriterijumaOptimalnosti(dozvola,TC,BrMedjuPodela);
...
%% --------------------------------------------------------------
function SumaTezinaTacaka(TC,MedjuPodela)
global SistemMrava ASPGOpcije
	for i=1:MedjuPodela
...
%% --------------------------------------------------------------
function SumaKriterijumaOptimalnosti(MDV,TC,MedjuPodela)
global SistemMrava ASPGOpcije
	for i=1:MedjuPodela
